 ...
 * @todo NamingStrategyConversion of constantName from='Bolivia (Plurinational State of)' to='Boliviaplurinationalstateof'
 * @todo NamingStrategyConversion of constantName from='Bonaire, Sint Eustatius and Saba' to='Bonairesinteustatiusandsaba'
 * @todo NamingStrategyConversion of constantName from='Bosnia and Herzegovina' to='Bosniaandherzegovina'
 * @todo bei .harmonize darf das Space nicht weggenommen werden! Nur so kann ein weiterer NS eingereiht werden!
 *
...
public class NamingStrategyFactory {
    ...
    /**
     * simple camel case meaning that the first character will always uppered
     * and each character after a space or a underline will be uppered.
    ...
    /**
     * strict camel case meaning that each character after the first uppered one will be strictly lowered.
     */
    ...
    /**
     * Amazing strategy which converts special characters (eg.: circumflex, acute, grave, ..) to its latin derivative if possible
     * and also removes all special characters which are non allowed to be used in a java variable name.
     */
    ...
    /**
     * Amazing strategy which converts special characters (eg.: circumflex, acute, grave, ..) to its latin derivative if possible
     * Special characters will be left alone.
     */
    public static NamingStrategy replaceAtoZ() {
        return value -> {
            ...
            for (int i = 0; i < value.length(); i++) {
                char current = value.charAt(i);
                int codePoint = value.codePointAt(i);
                if (current >= 192 && ((current != 215 || current != 247 || current != 451 || current != 760))) {
                    String name = Character.getName(value.codePointAt(i));
                    Matcher m = pattern.matcher(name);
                    if (m.matches()) {
                        String capital = m.group(2);
        ...
        return value -> {
            ...
            for (int i = 0; i < value.length(); i++) {
                ...
                if (current == 32 || current == 45 || current == 46 || current == 95) {
                    ...
                    buffer.append(current);
                } else if ((current >= 48 && current <= 57) ||(current >= 65 && current <= 90) || (current >= 97 && current <= 122)) {//A-Z
                    buffer.append(current);
                }
            }
            return buffer.toString();
    ...
    /**
     * Bolivia (Plurinational State of)        --> Plurinational State of
