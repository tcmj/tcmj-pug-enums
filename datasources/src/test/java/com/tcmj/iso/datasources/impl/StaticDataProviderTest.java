...
public class StaticDataProviderTest {
    ...
    public void overallTestWithSubfields() throws Exception {
        ...
        assertThat("getValue1", Arrays.toString(data.getData().entrySet().iterator().next().getValue().getValue()), equalTo("[30370000, 20.4, AF]"));
        assertThat("getValue1+2", Arrays.toString(data.getData().values().stream().map(e->Arrays.toString(e.getValue())).toArray()), equalTo("[[30370000, 20.4, AF], [123123123, 55.22, AN]]"));
    }
